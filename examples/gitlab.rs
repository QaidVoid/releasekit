//! Example: fetch and list releases from a GitLab repository.
//!
//! Usage:
//!   cargo run --example gitlab -- owner/repo
//!   cargo run --example gitlab -- owner/repo v1.0.0
//!   cargo run --example gitlab -- https://gitlab.com/owner/repo
...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("usage: gitlab <owner/repo | url> [tag]");
        std::process::exit(1);
    }
    ...
    let releases = match gl.fetch_releases(&project, tag.as_deref()) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("error: {e}");
            std::process::exit(1);
...
fn parse_input(input: &str, tag_arg: Option<&str>) -> (String, Option<String>) {
    if let Some(url) = PlatformUrl::parse(input) {
        match url {
            PlatformUrl::GitLab { project, tag } => (project, tag.or(tag_arg.map(String::from))),
            _ => {
                eprintln!("error: only GitLab URLs are supported in this example");
                std::process::exit(1);
            }
        }
    } else {
